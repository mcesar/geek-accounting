...
function Database() {
	...
	var setupReplicaSet, setupDatabase, db, isOpening, isOpen;
...
Database.prototype.open = function(callback) {
	var that = this;
	if (this.isOpening || this.isOpen) { return callback(); }
	this.isOpening = true;
	...
	this.conn().open(function(err, db) {
		...
		if (err) { return callback(err); }
		if (config.dbUser) {
			db.authenticate(config.dbUser, config.dbPassword, function (err) {
				if (err) { return callback(err); }
				afterOpen(db);
			});
		} else {
...
Database.prototype.find = function(collectionName, where, callback) {
	if (!callback) {
		callback = where;
		where = undefined;
	}
...
Database.prototype.remove = function(collectionName, where, callback) {
	this.conn().collection(collectionName, function(err, collection) {
		if (err) { return callback(err); }
		collection.remove(where, function (err, item) {
			if (err) { return callback(err); }
			callback(null, item);
		});
	});
